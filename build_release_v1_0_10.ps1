...
$buildRoot = 'C:\SYNTECBuild\InvoiceManager-v1.0.10'
$distRoot = 'C:\SYNTECBuild\dist-v1.0.10'
$releaseZip = Join-Path $sourceRoot 'release\SYNTEC-InvoiceManager-v1.0.10.zip'
...
$manual = Get-ChildItem -Path (Join-Path $sourceRoot 'docs') -Filter '*-v1.0.10.docx' -File | Select-Object -First 1
...
if ($version.FileVersion -notmatch '^1\.0\.10\.0') { throw 'Unexpected file version.' }
...
Copy-Item -LiteralPath $manual.FullName -Destination $appRoot -Force
Copy-Item -LiteralPath $config -Destination $internal -Force
